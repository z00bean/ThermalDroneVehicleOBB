cd ../..
export VIDEO_DIR="/work/pi_yuanchang_xie_uml_edu/zubin/DATA/THERMAL/VIDEOS/Medford/WORKZONE/Week_2"
...
do
  ...
  dest_file="runs/TRACK-Medford-WORKZ/Week_2-DICTLabels"
  echo $(basename "$f")
  ...
  echo
  echo "ffmpeg -i "$dest_file/$(basename "$f")"/$(basename "$f" .mp4)".avi" -preset slow -crf 29 "$dest_file/$(basename "$f")"/$(basename "$f")""" -loglevel warning
  echo
  echo Parallely ffmpeg converting video and creating dict_labels
  ffmpeg -i "$dest_file/$(basename "$f")"/$(basename "$f" .mp4)".avi" -preset slow -crf 29 "$dest_file/$(basename "$f")"/$(basename "$f")"" -loglevel warning&
